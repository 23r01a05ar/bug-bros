  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        ...
        // For creating, user must first sign up via Supabase Auth
        // This creates the profile entry — the auth user must exist
    ...
    <div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((n) => (
            <div key={n} className="skeleton" style={{ height: '52px' }} />
          ))}
        </div>
      ) : (
        ...
        <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <FileUpload
              ...
              onUpload={(url) => setForm({ ...form, resume_url: url })}
              label="Resume"
              accept=".pdf,.doc,.docx"
            />
          </div>
