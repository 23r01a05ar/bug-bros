      ...
      if (editing) {
        ...
        if (contributors.length > 0) {
          await supabase.from('contributors').insert(
            contributors.map((uid) => ({ project_id: editing.id, user_id: uid }))
          );
        }
        ...
        if (contributors.length > 0 && newProject) {
          await supabase.from('contributors').insert(
            contributors.map((uid) => ({ project_id: newProject.id, user_id: uid }))
          );
        }
      ...
      {loading ? (
        ...
        <div className="card p-0 overflow-hidden">
          <DataTable
            ...
            actions={(item) => (
              <>
                <button onClick={() => openEdit(item)} className="p-1.5 text-[var(--color-muted)] hover:text-[var(--color-accent)] transition-colors">
                  <Pencil size={16} />
                </button>
